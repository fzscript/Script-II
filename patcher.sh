...
\e[1;33m-o, --offline     \e[1;34mJalankan tanpa pembaruan Patcher\e[0m
...
do
    ...
    "-r" | "--reinstall")
                        if ping -c 1 google.com >/dev/null 2>&1; then
                            cd "$path"/.. && rm -rf "$path" && git clone --depth=1 https://github.com/fzproject/Patcher.git && patcher && exit
                        else
...
checkproject() {
    if [ -d "$path" ]; then
        ...
        y | Y)
            rm -rf "$path"
            git clone --depth=1 https://github.com/fzproject/Patcher.git && patcher && exit
            ;;
        n | N)
